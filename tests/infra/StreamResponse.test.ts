      ...
      getReader: () => {
        ...
        return {
          read() {
            ...
            : Promise.resolve({ value: new TextEncoder().encode(rawData[i]) })
          },
        };
      }
    }
...
it("log error on unknown stream event", async () => {
  ...
  const stream = new StreamResponse(mockResponse([
    'event: someRandomEvent\ndata: test\n\n',
  ]))
  await stream.consume()
  expect(errorSpy).toBeCalledWith('Stream Error: Unknown Event "someRandomEvent" with data: test')
})
...
it("log error on unknown stream data", async () => {
  ...
  await stream.consume()
  expect(errorSpy).toBeCalledWith("Stream Error: Unknown data:", "name: test\ninfo: mock\n\n")
})
...
it("log error when attempt to read stream data multiple times", async () => {
  ...
  await stream.consume()
  expect(errorSpy).toBeCalledWith("Stream Error: data has been consumed from the stream. Cannot be read again.")
})